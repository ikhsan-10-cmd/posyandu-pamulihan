...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $action = $_POST['action'];
    if ($action === 'add') {
        $stmt = $db->prepare("INSERT INTO ibu_hamil_2 (nama_ibu_hamil, nik, tempat_tanggal_lahir_ibu, nama_suami, nik_suami, tempat_tanggal_lahir_suami, alamat) VALUES (?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([
        ...
        $stmt = $db->prepare("UPDATE ibu_hamil_2 SET nama_ibu_hamil = ?, nik = ?, tempat_tanggal_lahir_ibu = ?, nama_suami = ?, nik_suami = ?, tempat_tanggal_lahir_suami = ?, alamat = ? WHERE id_ibu = ?");
        $stmt->execute([
            $_POST['nama_ibu_hamil'],
            $_POST['nik'],
            $_POST['tempat_tanggal_lahir_ibu'],
        ...
        $stmt = $db->prepare("DELETE FROM ibu_hamil_2 WHERE id_ibu = ?");
        $result = $stmt->execute([$_POST['id_ibu']]);
        if ($result) {
            // Hapus juga data catatan kehamilan terkait
            $stmt = $db->prepare("DELETE FROM catatan_kehamilan_2 WHERE id_ibu = ?");
...
$ibu_hamil = null;
if (isset($_GET['edit'])) {
    $result = getIbuHamil2AndCatatanKehamilan($db, $_GET['edit']);
    if (!empty($result)) {
        $ibu_hamil = $result[0];
    }
}
...
$stmt = $db->query("SELECT * FROM ibu_hamil_2");
...
<?php include __DIR__ . '/../pos_2/footer_ih_p2.php'; ?>