...
define('TABLE_IBU_HAMIL_2', 'ibu_hamil_2');
define('TABLE_CATATAN_KEHAMILAN_2', 'catatan_kehamilan_2');
...
// Menangani pengiriman formulir untuk menambah, mengedit, dan menghapus catatan kehamilan 2
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    switch ($action) {
        ...
        case 'edit':
            ...
            if ($action === 'add') {
                $sql = "INSERT INTO " . TABLE_CATATAN_KEHAMILAN_2 . " (id_ibu, hamil_keberapa, hpht, hpl, usia_kehamilan, status_kehamilan, tinggi_badan, berat_badan, lila, laboratorium, imunisasi, mendapatkan_bantuan, mempunyai_bpjs, bulan) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            } else {
                $data['id_kehamilan'] = filter_var($_POST['id_kehamilan'], FILTER_VALIDATE_INT);
                $sql = "UPDATE " . TABLE_CATATAN_KEHAMILAN_2 . " SET id_ibu = ?, hamil_keberapa = ?, hpht = ?, hpl = ?, usia_kehamilan = ?, status_kehamilan = ?, tinggi_badan = ?, berat_badan = ?, lila = ?, laboratorium = ?, imunisasi = ?, mendapatkan_bantuan = ?, mempunyai_bpjs = ?, bulan = ? WHERE id_kehamilan = ?";
            }
        ...
        case 'delete':
            ...
            if ($id_kehamilan) {
                $stmt = $db->prepare("DELETE FROM " . TABLE_CATATAN_KEHAMILAN_2 . " WHERE id_kehamilan = ?");
...
$catatan_kehamilan_2 = null;
if (isset($_GET['edit'])) {
    $id_kehamilan = filter_var($_GET['edit'], FILTER_VALIDATE_INT);
    if ($id_kehamilan) {
        $stmt = $db->prepare("SELECT * FROM " . TABLE_CATATAN_KEHAMILAN_2 . " WHERE id_kehamilan = ?");
        $stmt->execute([$id_kehamilan]);
        $catatan_kehamilan_2 = $stmt->fetch(PDO::FETCH_ASSOC);
    }
}

// Mengambil daftar semua ibu hamil untuk dropdown
$stmt = $db->query("SELECT id_ibu, nama_ibu_hamil FROM " . TABLE_IBU_HAMIL_2);
$ibu_hamil_list = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Mengambil daftar semua catatan kehamilan
$stmt = $db->query("SELECT c.*, i.nama_ibu_hamil FROM " . TABLE_CATATAN_KEHAMILAN_2 . " c JOIN " . TABLE_IBU_HAMIL_2 . " i ON c.id_ibu = i.id_ibu ORDER BY c.id_kehamilan ASC");
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="">
                        <input type="hidden" name="action" value="<?php echo isset($catatan_kehamilan_2) ? 'edit' : 'add'; ?>">
                        <?php if (isset($catatan_kehamilan_2)): ?>
                            <input type="hidden" name="id_kehamilan" value="<?php echo $catatan_kehamilan_2['id_kehamilan']; ?>">
                        <?php endif; ?>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="id_ibu" name="id_ibu" required>
                                <?php foreach ($ibu_hamil_list as $ibu): ?>
                                    <option value="<?php echo $ibu['id_ibu']; ?>" <?php echo (isset($catatan_kehamilan_2) && $catatan_kehamilan_2['id_ibu'] == $ibu['id_ibu']) ? 'selected' : ''; ?>>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control" id="hamil_keberapa" name="hamil_keberapa" 
                                value="<?php echo isset($catatan_kehamilan_2['hamil_keberapa']) ? $catatan_kehamilan_2['hamil_keberapa'] : 1; ?>" 
                                required min="1" step="1">
                        </div>

                        <div class="mb-3">
                            <label for="hpht" class="form-label">HPHT</label>
                            <input type="date" class="form-control" id="hpht" name="hpht" value="<?php echo isset($catatan_kehamilan_2) ? $catatan_kehamilan_2['hpht'] : ''; ?>" required>
                        </div>

                        <div class="mb-3">
                            <label for="hpl" class="form-label">HPL</label>
                            <input type="date" class="form-control" id="hpl" name="hpl" value="<?php echo isset($catatan_kehamilan_2) ? $catatan_kehamilan_2['hpl'] : ''; ?>" required>
                        </div>

                        <div class="mb-3">
                            <label for="usia_kehamilan" class="form-label">Usia Kehamilan</label>
                            <input type="text" class="form-control" id="usia_kehamilan" name="usia_kehamilan" value="<?php echo isset($catatan_kehamilan_2) ? $catatan_kehamilan_2['usia_kehamilan'] : ''; ?>" required>
                        </div>

                        <div class="mb-3">
                            <label for="status_kehamilan" class="form-label">Status Kehamilan</label>
                            <input type="text" class="form-control" id="status_kehamilan" name="status_kehamilan" value="<?php echo isset($catatan_kehamilan_2) ? $catatan_kehamilan_2['status_kehamilan'] : ''; ?>" required>
                        </div>

                        <div class="mb-3">
                            <label for="tinggi_badan" class="form-label">Tinggi Badan (cm)</label>
                            <input type="number" step="0.1" class="form-control" id="tinggi_badan" name="tinggi_badan" value="<?php echo isset($catatan_kehamilan_2) ? $catatan_kehamilan_2['tinggi_badan'] : ''; ?>" required>
                        </div>

                        <div class="mb-3">
                            <label for="berat_badan" class="form-label">Berat Badan (kg)</label>
                            <input type="number" step="0.1" class="form-control" id="berat_badan" name="berat_badan" value="<?php echo isset($catatan_kehamilan_2) ? $catatan_kehamilan_2['berat_badan'] : ''; ?>" required>
                        </div>

                        <div class="mb-3">
                            <label for="lila" class="form-label">LILA (cm)</label>
                            <input type="number" step="0.1" class="form-control" id="lila" name="lila" value="<?php echo isset($catatan_kehamilan_2) ? $catatan_kehamilan_2['lila'] : ''; ?>" required>
                        </div>

                        <div class="mb-3">
                            <label for="laboratorium" class="form-label">Laboratorium</label>
                            <textarea class="form-control" id="laboratorium" name="laboratorium" required><?php echo isset($catatan_kehamilan_2) ? $catatan_kehamilan_2['laboratorium'] : ''; ?></textarea>
                        </div>

                        <div class="mb-3">
                            <label for="imunisasi" class="form-label">Imunisasi</label>
                            <input type="text" class="form-control" id="imunisasi" name="imunisasi" value="<?php echo isset($catatan_kehamilan_2) ? $catatan_kehamilan_2['imunisasi'] : ''; ?>" required>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="mendapatkan_bantuan" name="mendapatkan_bantuan" required>
                                <option value="1" <?php echo (isset($catatan_kehamilan_2) && $catatan_kehamilan_2['mendapatkan_bantuan'] == 1) ? 'selected' : ''; ?>>Ya</option>
                                <option value="0" <?php echo (isset($catatan_kehamilan_2) && $catatan_kehamilan_2['mendapatkan_bantuan'] == 0) ? 'selected' : ''; ?>>Tidak</option>
                            </select>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <option value="1" <?php echo (isset($catatan_kehamilan_2) && $catatan_kehamilan_2['mempunyai_bpjs'] == 1) ? 'selected' : ''; ?>>Ya</option>
                                <option value="0" <?php echo (isset($catatan_kehamilan_2) && $catatan_kehamilan_2['mempunyai_bpjs'] == 0) ? 'selected' : ''; ?>>Tidak</option>
                            </select>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="bulan" name="bulan" required>
                                <?php foreach ($bulan_list as $bulan): ?>
                                    <option value="<?php echo $bulan; ?>" <?php echo (isset($catatan_kehamilan_2) && $catatan_kehamilan_2['bulan'] == $bulan) ? 'selected' : ''; ?>>
                        ...
                        <button type="submit" class="btn btn-primary w-100">
                            <i class="fas fa-save me-2"></i><?php echo isset($catatan_kehamilan_2) ? 'Update' : 'Tambah'; ?> Catatan Kehamilan
                        </button>
                    </form>
                </div>
            </div>
