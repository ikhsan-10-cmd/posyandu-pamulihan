...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add') {
        $stmt = $db->prepare("INSERT INTO balita_4 (nama_balita, jenis_kelamin, nik, tanggal_lahir, berat_badan_lahir, nama_ayah, nama_ibu, alamat, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([
            $_POST['nama_balita'],
            $_POST['jenis_kelamin'],
            $_POST['nik'],
        ...
        $stmt = $db->prepare("UPDATE balita_4 SET nama_balita = ?, jenis_kelamin = ?, nik = ?, tanggal_lahir = ?, berat_badan_lahir = ?, nama_ayah = ?, nama_ibu = ?, alamat = ?, status = ? WHERE id_balita = ?");
        $stmt->execute([
            $_POST['nama_balita'],
            $_POST['jenis_kelamin'],
            $_POST['nik'],
        ...
        $stmt = $db->prepare("DELETE FROM balita_4 WHERE id_balita = ?");
        $result = $stmt->execute([$_POST['id_balita']]);
        if ($result) {
            // Hapus juga data pengukuran terkait
            $stmt = $db->prepare("DELETE FROM pengukuran_balita_4 WHERE id_balita = ?");
...
$balita = null;
if (isset($_GET['edit'])) {
    $stmt = $db->prepare("SELECT * FROM balita_4 WHERE id_balita = ?");
    $stmt->execute([$_GET['edit']]);
    $balita = $stmt->fetch(PDO::FETCH_ASSOC);
}

// Mengambil daftar semua balita
$balitas = $db->query("SELECT * FROM balita_4")->fetchAll(PDO::FETCH_ASSOC);
?>
...
<?php include __DIR__ . '/../pos_4/footer_balita_p4.php'; ?>